  ...
  <div class="bg-editor/50">
    ...
    <div class="flex items-center gap-0.5 px-1.5 py-0.5 flex-wrap">
      ...
      <button
        ...
        :disabled="undoCount < 2" title="Undo (Ctrl+Z)" @click="fire('undo')"
      ><Undo2 :size="13" /></button>
      <button
        class="p-1 rounded text-text-secondary hover:text-text-primary hover:bg-surface-hover cursor-pointer transition-colors disabled:opacity-30"
        :disabled="!redoCount" title="Redo (Ctrl+Y)" @click="fire('redo')"
      ...
      <button
        ...
        title="Strikethrough (Ctrl+U)" @click="fire('strikethrough')"
      ...
      <button
        class="p-1 rounded text-text-secondary hover:text-text-primary hover:bg-surface-hover cursor-pointer transition-colors"
        title="Link" @click="fire('link')"
      ><Link :size="13" /></button>
      <button
        v-if="mode === 'journal'"
        class="p-1 rounded text-text-secondary hover:text-text-primary hover:bg-surface-hover cursor-pointer transition-colors"
        title="Image (markdown)" @click="fire('image')"
      ><ImageIcon :size="13" /></button>
      <template v-if="mode === 'notes'">
        <button
          class="p-1 rounded text-text-secondary hover:text-text-primary hover:bg-surface-hover cursor-pointer transition-colors"
          title="Embed image" @click="fire('embedImage')"
        ><ImagePlus :size="13" /></button>
        <button
          class="p-1 rounded text-text-secondary hover:text-text-primary hover:bg-surface-hover cursor-pointer transition-colors"
          title="Embed audio" @click="fire('embedAudio')"
        ><Music :size="13" /></button>
        <button
          class="p-1 rounded text-text-secondary hover:text-text-primary hover:bg-surface-hover cursor-pointer transition-colors"
      ...
      <button
        ...
        title="Insert Emoji" @click="emit('toggleEmoji')"
      ...
      <template v-if="mode === 'journal'">
        <button
          ...
          title="Focus Mode" @click="emit('toggleFocus')"
        ...
        <button
          ...
          title="Typewriter Mode" @click="emit('toggleTypewriter')"
      ...
      <button
        ...
        title="Find & Replace (Ctrl+F)" @click="emit('toggleFind')"
    ...
    <div class="flex items-center gap-0.5 px-1.5 py-0.5 flex-wrap">
      <button
        ...
        title="Heading 1" @click="fire('h1')"
      ><Heading1 :size="13" /></button>
      <button
        class="p-1 rounded hover:bg-surface-hover cursor-pointer transition-colors"
        :class="activeFormats.has('h2') ? 'bg-accent/20 text-accent' : 'text-text-secondary hover:text-text-primary'"
        title="Heading 2" @click="fire('h2')"
      ><Heading2 :size="13" /></button>
      <button
        v-if="mode === 'notes'"
        class="p-1 rounded hover:bg-surface-hover cursor-pointer transition-colors"
        :class="activeFormats.has('h3') ? 'bg-accent/20 text-accent' : 'text-text-secondary hover:text-text-primary'"
        title="Heading 3" @click="fire('h3')"
      ><Heading3 :size="13" /></button>
      <span class="w-px h-4 bg-border mx-0.5" />
      <button
        class="p-1 rounded hover:bg-surface-hover cursor-pointer transition-colors"
        :class="activeFormats.has('ul') ? 'bg-accent/20 text-accent' : 'text-text-secondary hover:text-text-primary'"
        title="Bullet list" @click="fire('ul')"
      ><List :size="13" /></button>
      <button
        class="p-1 rounded hover:bg-surface-hover cursor-pointer transition-colors"
        :class="activeFormats.has('ol') ? 'bg-accent/20 text-accent' : 'text-text-secondary hover:text-text-primary'"
        title="Numbered list" @click="fire('ol')"
      ><ListOrdered :size="13" /></button>
      <button
        class="p-1 rounded hover:bg-surface-hover cursor-pointer transition-colors"
        :class="activeFormats.has('checklist') ? 'bg-accent/20 text-accent' : 'text-text-secondary hover:text-text-primary'"
        title="Checklist" @click="fire('checklist')"
      ...
      <button
        ...
        title="Blockquote" @click="fire('quote')"
      ...
      <button
        ...
        title="Align left" @click="fire('alignLeft')"
      ...
      <button
        ...
        title="Align center" @click="fire('alignCenter')"
      ...
      <button
        ...
        title="Align right" @click="fire('alignRight')"
      ...
      <button
        ...
        title="Align justify" @click="fire('alignJustify')"
      ...
      <button
        ...
        title="Highlight" @click="fire('highlight')"
      ><Highlighter :size="13" /></button>
      <button
        class="p-1 rounded text-text-secondary hover:text-text-primary hover:bg-surface-hover cursor-pointer transition-colors"
        title="Table" @click="fire('table')"
      ><Table :size="13" /></button>
      <button
        v-if="mode === 'journal'"
        class="p-1 rounded text-text-secondary hover:text-text-primary hover:bg-surface-hover cursor-pointer transition-colors"
        title="Horizontal rule" @click="fire('hr')"
    ...
    >
      <span class="text-[9px] uppercase tracking-wider text-text-muted font-semibold mr-1 select-none">Journal</span>
      <button
        v-for="emoji in journalEmojis" :key="emoji"
        class="w-6 h-6 flex items-center justify-center text-base hover:bg-surface-hover rounded cursor-pointer transition-colors"
        :title="`Insert ${emoji}`" @click="emit('quickEmoji', emoji)"
      >{{ emoji }}</button>
    </div>
  </div>
</template>
