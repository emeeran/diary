...
const autoOcrImages = useLocalStorage<boolean>(
  'lifelogr-auto-ocr-images',
  false,
)
...
const ttsVoice = useLocalStorage<string>(
  'lifelogr-tts-voice',
  'en-US-AvaNeural',
)
...
async function previewVoice() {
  if (ttsPreviewing.value && previewAudio) {
    previewAudio.pause()
  ...
  try {
    const { key } = await ttsApi.speakUrl(
      'This is how your journal will sound when read aloud.',
    )
    if (!key) {
      ttsPreviewing.value = false
      return
...
<template>
  <SettingsSection
    ...
    setting-key="Appearance"
    reset-label="Reset"
    @reset="resetAppearanceDefaults"
  >
    <SettingRow label="Dark mode">
      <template #icon>
        <component
          :is="ui.darkMode ? Moon : Sun"
          :size="13"
    ...
    <SettingRow :icon="Type" :label="`Font size (${ui.fontSize}px)`">
      <input
        type="range"
        :value="ui.fontSize"
        @input="ui.setFontSize(+($event.target as HTMLInputElement).value)"
        min="12"
        max="20"
        step="1"
  ...
  <SettingsSection
    ...
    :icon="Sliders"
    description="Writing behavior, search, and preferences"
    reset-label="Reset"
    @reset="resetEditorDefaults"
    card-class="p-3"
  >
    <SettingGroup label="Writing">
      <SettingRow
        :icon="Clock"
        :label="`Auto-save (${autosaveInterval}s)`"
      ...
      <SettingRow
        :icon="Eye"
        label="OCR language"
        description="Used when extracting text from attached images."
      >
        <select v-model="ocrLanguage" class="settings-select w-36">
          <option v-for="l in ocrLanguages" :key="l.value" :value="l.value">
            {{ l.label }}
          </option>
        </select>
      </SettingRow>
    ...
    <SettingGroup label="Search">
      ...
      <p class="text-[10.5px] text-text-muted pl-[31px] leading-snug">
        <span v-if="searchStore.searchMode === 'hybrid'"
          >Combines keyword and semantic search for best results.</span
        >
        <span v-else-if="searchStore.searchMode === 'keyword'"
          >Fast text matching. Works without AI models.</span
        >
        <span v-else
          >Finds entries by meaning, not just words. Requires an embedding
          model.</span
        >
      </p>
    </SettingGroup>
  ...
  <SettingsSection
    ...
    setting-key="Voice"
    reset-label="Reset"
    @reset="resetTTSDefaults"
  >
    <SettingRow
      :icon="Volume2"
      label="Voice"
      description="Voice used when reading entries aloud."
    >
      <select
        v-model="ttsVoice"
        class="settings-select max-w-44"
      ...
      >
        <option v-if="ttsVoicesLoading" disabled value="">
          Loading voices...
        </option>
        <template
          v-for="[locale, voices] in voicesByLocale(ttsVoices)"
          :key="locale"
        >
    ...
    <SettingRow
      indent
      :label="`Pitch (${ttsPitch > 0 ? '+' : ''}${ttsPitch} Hz)`"
  ...
  <AccordionItem
    ...
    :icon="Keyboard"
    description="Quick reference for editor shortcuts"
  >
    <div class="divide-y divide-border -m-3">
      ...
      >
        ...
        <kbd
          class="px-1.5 py-0.5 bg-surface-hover rounded-md text-[10px] font-mono text-text-muted border border-border"
          >{{ s.keys }}</kbd
        >
      </div>
    </div>
