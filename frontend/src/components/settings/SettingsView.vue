...
function migrateTabId(id: string): string {
  return (TAB_IDS as readonly string[]).includes(id)
    ? id
    : (LEGACY_TAB[id] ?? 'appearance')
}
const _storedTab = useLocalStorage<string>(
  'lifelogr-settings-tab',
  'appearance',
)
...
const index: SearchEntry[] = [
  ...
  },
  {
    tab: 'about',
    label: 'Updates',
    keywords: ['update', 'updates', 'check for updates', 'release'],
  },
  {
    tab: 'about',
    label: 'System setup',
