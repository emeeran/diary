...
const fileUrl = computed(() =>
  props.single ? props.single.src : current.value ? mediaApi.fileUrl(current.value.id) : '',
)
const activeType = computed(() => (props.single ? props.single.mediaType : current.value?.media_type ?? ''))
const activeName = computed(() => (props.single ? props.single.filename ?? 'media' : current.value?.filename ?? ''))
...
const zoomable = computed(() => isImage(activeType.value) || isVideo(activeType.value))
  ...
  <div class="fixed inset-0 z-50 bg-black/92 backdrop-blur-sm flex items-center justify-center select-none"
    ...
    <div class="absolute top-0 inset-x-0 z-20 flex items-center justify-between px-4 py-3 bg-gradient-to-b from-black/70 to-transparent">
      ...
      <div class="flex items-center gap-1.5">
        ...
        <a v-if="current || single" :href="fileUrl" :download="activeName" title="Download"
    ...
    <!-- Nav arrows -->
    <button v-if="!single && index > 0" class="nav-btn left-4" @click="prev"><ChevronLeft :size="26" /></button>
    <button v-if="!single && index < items.length - 1" class="nav-btn right-4" @click="next"><ChevronRight :size="26" /></button>
    ...
    <template v-if="current || single">
      ...
      <!-- Audio -->
      <div v-else-if="isAudio(activeType)" class="flex flex-col items-center gap-5 px-6">
        ...
        <p class="text-sm text-white/90 font-medium text-center max-w-md truncate">{{ activeName }}</p>
      ...
      <div v-else class="flex flex-col items-center gap-4 text-white px-6">
        ...
        <p class="text-sm text-white/80 text-center max-w-md">{{ activeName }}</p>
        <a :href="fileUrl" :download="activeName"
