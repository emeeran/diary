...
export const rewrite = (
  ...
  request<RewriteResponse>('/ai/rewrite', {
    method: 'POST',
    body: JSON.stringify({ text, style, instructions }),
  })

export const aiStatus = () => request<AIStatusResponse>('/ai/status')

export const suggestTags = (text: string) =>
  request<TagSuggestionResponse>('/ai/suggest-tags', {
    method: 'POST',
...
export const pullModel = (model: string) =>
  request<{ status: string; model: string }>(
    `/ai/pull-model?model=${encodeURIComponent(model)}`,
    { method: 'POST' },
  )
...
export const expand = (text: string) =>
  request<ExpandResponse>('/ai/expand', {
    method: 'POST',
    body: JSON.stringify({ text }),
  })
...
export const getProviderPresets = () =>
  request<ProviderPreset[]>('/ai/providers/presets')
...
export const testProvider = (id: number) =>
  request<{ status: string; model: string }>(`/ai/providers/${id}/test`, {
    method: 'POST',
  })
...
export const listProviderModels = (id: number) =>
  request<{ models: ProviderModel[] }>(`/ai/providers/${id}/models`).then(
    (r) => r.models,
  )
